...
int mode = 1; // Default: Autonomous Mode
...
void handleRoot() {
  ...
  html += "<p><b>Current Mode:</b> " + String((mode == 1) ? "Autonomous" : (mode == 2) ? "Web Control" : "Human Follow") + "</p>";
  html += "<button class='mode-btn' onclick=\"sendRequest('1')\">Autonomous Mode</button>";
  html += "<button class='mode-btn' onclick=\"sendRequest('2')\">Web Control Mode</button>";
  html += "<button class='mode-btn' onclick=\"sendRequest('3')\">Human Follow Mode</button><br><br>";

  if (mode == 2) { // Web Control Mode
  ...
  html += "<script>";
  html += "function sendRequest(cmd){fetch('/control?cmd='+cmd);}"; // Send commands to ESP
...
void handleControl() {
  if (server.hasArg("cmd")) {
    ...
    if (cmd.startsWith("SPEED")) { // Handle speed changes
      int newSpeed = cmd.substring(5).toInt();
      NodeMCU.write('V');  // 'V' signals speed change
      NodeMCU.write(newSpeed);
    } else {
      char command = cmd[0];
      if (command == '1' || command == '2' || command == '3') mode = command - '0'; // Update mode
